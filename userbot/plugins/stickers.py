            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    if not await stickerset_exists(bot_conv, packshortname):
                        ...
                        response = await bot_conv.get_response()
                        if "Sorry" in response.text:
                            await event.edit(f"**FAILED**! @Stickers replied: {response.text}")# Copyright (C) 2019 The Raphielscape Company LLC.
#
...
async def kang(args):
    ...
    if photo:
        ...
        packnick = (
            f"{telename} Vol.{pack}"
            if telename
            else f"{user.username}'s Sylix Self Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
